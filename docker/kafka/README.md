...
Add more brokers:
when running docker-compose-multi-broker.yml, you can run the following command to add another kafka broker.
...
The default ```docker-compose.yml``` launches a singl kafka broker. 
The alternative docker-compose-multi-broker should be seen as the composition for a multi broker kafka cluster.
Using docker-compose-multi-broker, by default each broker will get a new port number and broker id on restart.
Depending on your use case this might not be desirable. If you need to use specific ports and broker ids,
modify the docker-compose configuration accordingly.
