...
Latency is the JSON record create time minus the UDP send time. Pass --skip-raw when the ODE
publishes topic.OdeBsmJson directly and does not write topic.OdeRawEncodedBSMJson.
...
def main():
    ...
    parser.add_argument("--skip-raw", action="store_true",
                        help="consume only the JSON topic; use when ODE skips raw encoded topics")
    ...
    def capture_until(required, deadline):
        ...
        while time.monotonic() < deadline:
            # Direct JSON publishes Ode JSON and leaves the raw topic empty.
            if not args.skip_raw and len(raw) == 0 and len(output) >= required:
                args.skip_raw = True
