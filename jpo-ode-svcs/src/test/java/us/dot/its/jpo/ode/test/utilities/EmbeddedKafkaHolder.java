 ...
 *
 * <p>
 * This class initializes an embedded Kafka broker with a specified configuration, which includes a
 * single broker and auto-start enabled. The broker list property is set to
 * "spring.kafka.bootstrap-servers".
 *
 * </p>
 * The embedded Kafka instance is lazily started when the getEmbeddedKafka method is called for the
 * first time. If the broker fails to start, a KafkaException is thrown.
 *
 * </p>
 * The class is designed to be non-instantiable with a private constructor.
 */
@Slf4j
public final class EmbeddedKafkaHolder {
  ...
  /**
   * Adds one or more topics to the embedded Kafka broker instance. Each topic will be created with
   * a replication factor of 1 and a single partition. If a topic already exists, no action is
   * taken, and the exception is ignored.
   *
