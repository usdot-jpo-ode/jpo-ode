...
class TimReceiverTest {
  ...
  private static final String BASE = "src/test/resources/us/dot/its/jpo/ode/udp/tim/";
  ...
  @BeforeAll
  void startReceiver() {
    timReceiver = new TimReceiver(udpReceiverProperties.getTim(), decodeService);
  ...
  void testRawJ2735() throws Exception {
    String content = Files.readString(Paths.get(BASE + "TimReceiverTest_ValidTIM.txt"));
    new TestUDPClient(udpReceiverProperties.getTim().getReceiverPort()).send(content);
    verify(decodeService, timeout(5000)).decode(any(DatagramPacket.class), eq(SupportedMessageType.TIM));
  }

  @Test
  void testWithSignature() throws Exception {
    String content = Files.readString(Paths.get(BASE + "TimReceiverTest_ValidTIM_WithSignature.txt"));
    new TestUDPClient(udpReceiverProperties.getTim().getReceiverPort()).send(content);
    verify(decodeService, timeout(5000)).decode(any(DatagramPacket.class), eq(SupportedMessageType.TIM));
  }
}
