...
class RtcmReceiverTest {
  ...
  private static final String BASE = "src/test/resources/us/dot/its/jpo/ode/udp/rtcm/";
  ...
  @BeforeAll
  void startReceiver() {
    rtcmReceiver = new RtcmReceiver(udpReceiverProperties.getRtcm(), decodeService);
  ...
  void testRawJ2735() throws Exception {
    String content = Files.readString(Paths.get(BASE + "RtcmReceiverTest_ValidRTC.txt"));
    new TestUDPClient(udpReceiverProperties.getRtcm().getReceiverPort()).send(content);
    verify(decodeService, timeout(5000)).decode(any(DatagramPacket.class), eq(SupportedMessageType.RTCM));
  }

  @Test
  void testWithSignature() throws Exception {
    String content = Files.readString(Paths.get(BASE + "RtcmReceiverTest_ValidRTC_WithSignature.txt"));
    new TestUDPClient(udpReceiverProperties.getRtcm().getReceiverPort()).send(content);
    verify(decodeService, timeout(5000)).decode(any(DatagramPacket.class), eq(SupportedMessageType.RTCM));
  }
}
