...
class PsmReceiverTest {
  ...
  private static final String BASE = "src/test/resources/us/dot/its/jpo/ode/udp/psm/";
  ...
  @BeforeAll
  void startReceiver() {
    psmReceiver = new PsmReceiver(udpReceiverProperties.getPsm(), decodeService);
  ...
  void testRawJ2735() throws Exception {
    String content = Files.readString(Paths.get(BASE + "PsmReceiverTest_ValidPSM.txt"));
    new TestUDPClient(udpReceiverProperties.getPsm().getReceiverPort()).send(content);
    verify(decodeService, timeout(5000)).decode(any(DatagramPacket.class), eq(SupportedMessageType.PSM));
  }

  @Test
  void testWithSignature() throws Exception {
    String content = Files.readString(Paths.get(BASE + "PsmReceiverTest_ValidPSM_WithSignature.txt"));
    new TestUDPClient(udpReceiverProperties.getPsm().getReceiverPort()).send(content);
    verify(decodeService, timeout(5000)).decode(any(DatagramPacket.class), eq(SupportedMessageType.PSM));
  }
}
