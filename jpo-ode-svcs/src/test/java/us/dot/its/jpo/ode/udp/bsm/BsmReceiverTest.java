...
class BsmReceiverTest {
  ...
  private static final String BASE = "src/test/resources/us/dot/its/jpo/ode/udp/bsm/";
  ...
  @BeforeAll
  void startReceiver() {
    bsmReceiver = new BsmReceiver(udpReceiverProperties.getBsm(), decodeService);
  ...
  void testRawJ2735() throws Exception {
    String content = Files.readString(Paths.get(BASE + "BsmReceiverTest_ValidBSM.txt"));
    new TestUDPClient(udpReceiverProperties.getBsm().getReceiverPort()).send(content);
    verify(decodeService, timeout(5000)).decode(any(DatagramPacket.class), eq(SupportedMessageType.BSM));
  }

  @Test
  void testWithSignature() throws Exception {
    String content = Files.readString(Paths.get(BASE + "BsmReceiverTest_ValidBSM_WithSignature.txt"));
    new TestUDPClient(udpReceiverProperties.getBsm().getReceiverPort()).send(content);
    verify(decodeService, timeout(5000)).decode(any(DatagramPacket.class), eq(SupportedMessageType.BSM));
  }
}
