...
/**
 * The Asn1DecodedDataRouter is responsible for routing all decoded messages
 * from the topic.Asn1DecoderOutput Kafka topic to become deserialized POJOs
 * and be written back to Kafka.
 **/
@Slf4j
public class Asn1DecodedDataRouter extends AbstractSubscriberProcessor<String, String> {
  ...
  /**
   * Creates a copy of Asn1DecodedDataRouter to actively consume from Kafka and write the
   * deserialized output to the specified POJO and JSON topics.
   *
   * @param odeKafkaProperties The Kafka properties used to consume and produce to Kafka
   * @param pojoTopics The specified POJO topics to write the deserialized POJO output to
   * @param jsonTopics The specified JSON topics to write the deserialized JSON output to
   **/
  public Asn1DecodedDataRouter(OdeKafkaProperties odeKafkaProperties, PojoTopics pojoTopics, 
    ...
    this.timProducer = MessageProducer.defaultStringMessageProducer(
        ...
        odeKafkaProperties.getKafkaType(),
        odeKafkaProperties.getDisabledTopics());
    this.spatProducer = MessageProducer.defaultStringMessageProducer(
        odeKafkaProperties.getBrokers(),
        odeKafkaProperties.getKafkaType(),
        odeKafkaProperties.getDisabledTopics());
    this.ssmProducer = MessageProducer.defaultStringMessageProducer(
        odeKafkaProperties.getBrokers(),
        odeKafkaProperties.getKafkaType(),
        odeKafkaProperties.getDisabledTopics());
    this.srmProducer = MessageProducer.defaultStringMessageProducer(
        odeKafkaProperties.getBrokers(),
        odeKafkaProperties.getKafkaType(),
        odeKafkaProperties.getDisabledTopics());
    this.psmProducer = MessageProducer.defaultStringMessageProducer(
        odeKafkaProperties.getBrokers(),
        odeKafkaProperties.getKafkaType(),
        odeKafkaProperties.getDisabledTopics());
  }
  ...
  private void routePSM(String consumedData, RecordType recordType) 
  ...
  private void routeSRM(String consumedData, RecordType recordType) 
      throws XmlUtils.XmlUtilsException {
  ...
  private void routeSSM(String consumedData, RecordType recordType) 
      throws XmlUtils.XmlUtilsException {
  ...
  private void routeTIM(String consumedData, RecordType recordType) 
      throws XmlUtils.XmlUtilsException {
    String odeTimData = OdeTimDataCreatorHelper.createOdeTimDataFromDecoded(
        consumedData).toString();
    switch (recordType) {
  ...
  private void routeBSM(String consumedData, RecordType recordType) 
      throws XmlUtils.XmlUtilsException {
    ...
    switch (recordType) {
      case bsmLogDuringEvent -> bsmProducer.send(pojoTopics.getBsmDuringEvent(), 
        getRecord().key(), odeBsmData);
